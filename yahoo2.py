...
class YahooAPI(object):
    ...
    def request(self, request_str, params=None):
    ...
    @staticmethod
    def get_league_key(league_id):
        return "{game_key}.l.{league_id}". format(game_key="nfl", league_id=league_id)

def run():
    client = YahooAPI("yahoo_key.json", tokenfile="token.txt")
    key = YahooAPI.get_league_key(LEAGUE_ID)
    path = "/fantasy/v2/league/{}/players".format(key)
    res = client.request(path, {"count": 50})
    data = res.json()
    print json.dumps(data[u'fantasy_content'][u'league'][1][u'players'], indent=2)
